...
class HandlerRegistry:
    ...
    def __init__(self, handlers: List[THandler]):
        self.handlers = {handler.get_request_type(): handler for handler in handlers}

    def get_handler(self, request_type: Type[TRequest]) -> THandler:
        return self.handlers.get(request_type)
    
    def register_handler(self, handler: THandler):
        self.handlers[handler.get_request_type()] = handler

    def decorate_handler(self, handler: THandler):
        self.register_handler(handler)
        return handler
    
    def dispatch(self, request: TRequest, *args, **kwargs) -> TResponse:
        handler = self.get_handler(type(request))
        return handler.handle(request, *args, **kwargs)
